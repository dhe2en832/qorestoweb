...
export default function BQOHome() {
    ...
    async function getDatas() {
        const res = await fetch(process.env.PUBLIC_URL + '/data/bqo_test.json')
    ...
    // List -> Category
    const [tabValue, setTabValue] = useState("cat_all");
    const handleTabChange = (event, newValue) => {
        ...
        switch (newValue) {
            ...
            case "cat_promos":
                ...
                break;
            case "cat_foods":
                async function foodsCategory() {
                    const resJson = await getDatas()
                    datasFilter = resJson.datas.filter((data) => data.category === "foods");
                    setLists(datasFilter)
                }
                foodsCategory();
                break;
            case "cat_drinks":
                async function drinksCategory() {
                    const resJson = await getDatas()
                    datasFilter = resJson.datas.filter((data) => data.category === "drinks");
                    setLists(datasFilter)
                }
                drinksCategory();
                break;
            case "cat_cake":
                async function cakeCategory() {
                    const resJson = await getDatas()
                    datasFilter = resJson.datas.filter((data) => data.category === "cake");
                    setLists(datasFilter)
                }
                cakeCategory();
                break;
            case "cat_others":
                async function othersCategory() {
                    const resJson = await getDatas()
                    datasFilter = resJson.datas.filter((data) => data.category === "others");
                    setLists(datasFilter)
                }
                othersCategory();
                break;
    ...
    return (
        <>
                ...
                <AppBar position="fixed" sx={styles.appBar}>
                    <Toolbar>
                        <Grid container justifyContent="space-between">
                            <Grid item xs={1}>
                                <IconButton sx={styles.appBarIcon} onClick={() => { navigate("/") }}>
                                    <BackIcon />
                                </IconButton>
                            </Grid>
                            <Grid item xs={11}>
                                <InputBase onChange={handleChangeSearch} fullWidth={true} sx={styles.search} endAdornment={
                                    <IconButton sx={styles.appBarIcon}>
                                        <SearchIcon />
                                    </IconButton>
                                } type='search' placeholder='Cari Apa?' />
                            </Grid>
                        </Grid>
                    </Toolbar>
                </AppBar>
                    ...
                    <Box sx={{ bgcolor: 'background.paper', p: 1 }}>
                        ...
                        >
                            {categories.map((category) => (
                                <Tab key={category.id + "_key"} label={category.label} value={category.id}
                                    sx={{ display: category.id === "cat_none" ? 'none' : 'inline-flex' }}
                                />
                            ))}
                        </Tabs>
                    </Box>
                        ...
                        lists.length !== 0 ? lists.map((data) => (
                            <Paper key={data.id} sx={{
                                ...
                                <Grid container sx={{ px: 1, py: 0.2 }} justifyContent='flex-start' spacing={1}>
                                    ...
                                    <Grid item xs={7}>
                                        <Typography variant='body1' component='h2'>{data.name}</Typography>
                                        <Typography variant='caption' component='h3' color="#a7a7a7" lineHeight={1}>{data.desc}</Typography>
                                        {parseFloat(data.sellPrice) < parseFloat(data.price) && (
                                            <Typography variant={smUp ? 'body1' : 'body2'} fontWeight={500} component='span' color="#787878" mr={0.6} sx={{ textDecoration: 'line-through' }}>Rp {toCurrencyIDR(data.price)}</Typography>
                                        )}
                                        <Typography variant={smUp ? 'body1' : 'body2'} fontWeight={500} component='span'>Rp {toCurrencyIDR(data.sellPrice)}</Typography>
                                    </Grid>
                                    {isExistItem(data.id) ?
                                        (
                                            <Grid container item xs={12} justifyContent='flex-end' mb={1}>
                                                <Button variant='outlined' color={isNoteExist(data.id) ? 'success' : 'primary'} sx={styles.noteButton}
                                                    onClick={
                                                        (event) => {
                                                            event.stopPropagation();
                                                ...
                                                <TextField onChange={(event) => changeQtyItem(event, data)} onClick={(event) => selectedQtyItem(event)} value={cart[data.id].qty} variant="outlined" color="primary" sx={styles.qtyInput} InputProps={{
                                                    startAdornment: (
                                                        <IconButton sx={styles.adornIcon} onClick={(event) => decreaseQtyItem(event, data)}>
                                                            <RemoveIcon />
                                                        </IconButton>
                                                    ),
                                                    endAdornment: (
                                                        <IconButton sx={styles.adornIcon} onClick={(event) => increaseQtyItem(event, data)}>
                                                            <AddIcon />
                                                        </IconButton>
                                                    )
                                                }} />
                                            ...
                                            <Grid container item xs={12} justifyContent='flex-end' mb={1}>
                                                <Button variant="contained" onClick={(event) => addItem(event, data)}>Tambah</Button>
                                            </Grid>
                                        )}
                                </Grid>
                            </Paper>
            ...
            {/* Info */}
            <Dialog maxWidth="md" key="InfoDlg" open={showDialog.isShow && !showDialog.isForm} onClose={handleCloseDialog}>
                {showDialog.isShow && searchInfoDialog(showDialog.accessorID).map((data) => (
                    <div key={data.id + "dlg"}>
                        <DialogContent>
                            <img style={{ maxHeight: '60vh', width: '100%', objectFit: 'cover', borderRadius: '0.5rem' }} src={data.picture} alt={data.name} />
                            <Typography variant="h6" component="h2">{data.name}</Typography>
                            <Typography variant="caption" component="h3" color="#a7a7a7">{data.desc}</Typography>
                            <Grid container justifyContent="space-between">
                                <Grid item><Typography variant={smUp ? 'body1' : 'body2'} fontWeight={500} component='span'>Harga</Typography></Grid>
                                <Grid item>
                                    {parseFloat(data.sellPrice) < parseFloat(data.price) && (
                                        <Typography variant={smUp ? 'body1' : 'body2'} fontWeight={500} component='span' color="#787878" mr={0.6} sx={{ textDecoration: 'line-through' }}>Rp {toCurrencyIDR(data.price)}</Typography>
                                    )}
                        ...
                        <DialogActions>
                            <Button mr={2} variant="contained" onClick={(event) => handleAddItemDlg(event, data)} disabled={isExistItem(data.id) ? true : false}>{isExistItem(data.id) ? <CartIcon /> : <CartLessIcon />}</Button>
                        ...
                        <div style={{ maxWidth: '550px', width: '100%', position: 'fixed', bottom: '0', padding: '1rem' }}>
                            <Grid container spacing={1} justifyContent='space-between' sx={{ m: 0, p: 2, backgroundColor: '#3f50b5', width: '100%', borderRadius: '5px', color: '#fff' }}>
                                <Grid item container xs={11} spacing={1} >
                                    ...
                                    <Grid item>
                                        <Typography variant={smUp ? "body1" : "body2"} fontWeight={500}>{calculateQtyItem()} Item</Typography>
                                    </Grid>
                                    <Grid item>
                                        <Typography variant={smUp ? "body1" : "body2"} fontWeight={500}>:</Typography>
                                    </Grid>
                                    <Grid item>
                                        <Typography variant={smUp ? "body1" : "body2"} fontWeight={500}>Rp {toCurrencyIDR(calculatePriceItem())}</Typography>
                                    </Grid>
                                </Grid>
